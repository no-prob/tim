		...
		do {
			...
			if (fname.substr(0, tdataDir.size()) == tdataDir && (fname.size() <= tdataDir.size() || fname.at(tdataDir.size()) == '/')) {
				...
				if (equal(tofname, updaterName)) { // bad update - has Updater.exe - delete all dir
					...
					delFolder();
					return false;
				} else if (equal(tofname, updateTo + L"Nim.exe") && exeName != L"Nim.exe") {
...
void updateRegistry() {
	if (versionNum && versionNum != 0x7FFFFFFF) {
		...
		HKEY rkey;
		LSTATUS status = RegOpenKeyEx(HKEY_CURRENT_USER, L"Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\{7E477388-B86E-11ED-9062-EB575CEF4FD3}_is1", 0, KEY_QUERY_VALUE | KEY_SET_VALUE, &rkey);
...
static const WCHAR *_programName = L"Nim Desktop"; // folder in APPDATA, if current path is unavailable for writing
